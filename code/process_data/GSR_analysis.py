...
#     signal_f = mean_filter(signal,3*sampling_rate,1,sampling_rate,plt_flag)
    ...
    peaks,ends,starts=get_peak_intervals(signal_f,peaks,sampling_rate,plt_flag,tonic)
...
def get_peak_intervals(sig,peak_indexes,sampling_frequency,plt_flag,tonic):
    window_size = 4
    window_slide=1
    inertion=.01
    ends = []
...
#         print(i,inc_duration,dec_duration,inc_amplitude,dec_amplitude)
    ...
    if(plt_flag and len(peak_indexes)>0):
        ...
        plt.plot(tonic, alpha=0.5, color='green',label="GSR tonic component")
        plt.legend()
        plt.show()
...
def get_peak_intervals_features(sig,peak_indexes,starts,ends,sampling_frequency):
    if(len(peak_indexes)>0):
        ...
        num_features = 17
        gsr_peak_features = np.array([0.1]*num_features)
